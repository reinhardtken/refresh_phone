...
class PackageListTable : public CTPViewBase,
 ...
 public:
  ...
  PackageListTable(CTPTabbedPane *p, std::wstring const &);
  ...
  void OnUpdatePackageList(std::wstring const &, PointerWrapper<std::vector<ANDROID_PACKAGE_T>> const & p);
  
 private:
   MarginRateList list_;
   MarginRateMap map_;

   std::vector<ANDROID_PACKAGE_T> date_;
  ...
  DISALLOW_COPY_AND_ASSIGN(PackageListTable);
};
