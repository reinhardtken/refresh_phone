...
// There is a singleton read-only CommandParser that represents the command line
...
class PHONE_COMMON_EXPORT CommandParser {
 public:
#if defined(OS_WIN)
  ...
  explicit CommandParser(NoProgram no_program);

  // Construct a new command line with |program| as argv[0].
  explicit CommandParser(const FilePath& program);

  // Construct a new command line from an argument list.
  CommandParser(int argc, const CharType* const* argv);
  explicit CommandParser(const StringVector& argv);

  ~CommandParser();

  // Initialize the current process CommandParser singleton. On Windows, ignores
  ...
  // Destroys the current process CommandParser singleton. This is necessary if
  // you want to reset the base library to its initial state (for example, in an
  // outer library that needs to be able to terminate, and be re-initialized).
  // If Init is called only once, as in main(), Reset() is not necessary.
  //static void Reset();

  // Get the singleton CommandParser representing the current process's
  // command line. Note: returned value is mutable, but not thread safe;
  // only mutate if you know what you're doing!
  //static CommandParser* ForCurrentProcess();

#if defined(OS_WIN)
  static CommandParser FromString(const std::wstring& command_line);
#endif
  ...
  void CopySwitchesFrom(const CommandParser& source,
  ...
  void AppendArguments(const CommandParser& other, bool include_program);
 ...
 private:
  ...
  //   CommandParser cl(*CommandParser::ForCurrentProcess());
  //   cl.AppendSwitch(...);

  // The singleton CommandParser representing the current process's command line.
  static CommandParser* current_process_commandline_;
