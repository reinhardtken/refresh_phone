...
//
//   CommonThread::PostTask(CommonThread::IO, FROM_HERE, task);
//
...
class PHONE_COMMON_EXPORT CommonThread : public base::Thread {
public:
  ...
  enum ID {
    ...
    // Used to launch and terminate Common processes.
    PROCESS_LAUNCHER,
  ...
  // Construct a CommonThread with the supplied identifier.  It is an error
  // to construct a CommonThread that already exists.
  explicit CommonThread(ID identifier);

  // Special constructor for the main (UI) thread and unittests. We use a dummy
  // thread here since the main thread already exists.
  CommonThread(ID identifier, MessageLoop* message_loop);
  ...
  //           Foo, CommonThread::DeleteOnIOThread> {
  //
  // ...
  //  private:
  //   friend struct CommonThread::DeleteOnThread<CommonThread::IO>;
  ...
  struct DeleteOnCTPThread : public DeleteOnThread<CTP> { };
...
private:
  ...
  // This lock protects |common_threads_|.  Do not read or modify that array
  // without holding this lock.  Do not block while holding this lock.
  static base::Lock lock_;

  // An array of the CommonThread objects.  This array is protected by |lock_|.
  // The threads are not owned by this array.  Typically, the threads are owned
  // on the UI thread by the g_common_process object.  BrowserThreads remove
  // themselves from this array upon destruction.
  static CommonThread* common_threads_[ID_COUNT];
};
