...
std::wstring CAdbInterface::adb_exe_path_;
...
ANDROID_OS_INFO::ANDROID_OS_INFO(CString release)
{
	...
	for (next++; next != CAdbInterface::s_mapAndroidOsInfos.end(); next++)
	{
		...
		//uint32 len = next->first.size() > release.size() ? release.size() : next->first.size();
		//if ((base::strncasecmp(next->first.c_str(), release.c_str(), len)))
		cit = next;
	}
	*this = cit->second;
}
...
BOOL CAdbInterface::GetDevices()
{
	...
	if (para.strReturn.Find(TEXT("系统找不到指定的路径")) != -1) {
		//说明没找到adb
		return FALSE;
	}
	std::wstring result = para.strReturn.GetBuffer();
	para.strReturn.ReleaseBuffer();
	//error: could not install *smartsocket* listener: cannot bind to 127.0.0.1:5037: 
	//通常每个套接字地址(协议/网络地址/端口)只允许使用一次。 (10048)
	if (StartsWith(result, L"error:", false)) {
		return FALSE;
	}
...
#define SendDebugMessage(no, dbginfo) do {\
LOG(INFO)<<L"adb info: "<<no<<L"  "<<dbginfo;\
} while (0)
...
BOOL CAdbInterface::_CreateAdbProcess(P_PARAM_T para)
{
	...
	SendDebugMessage(no, para->strCmd);
	CString strEchoFile;
	para->strCmd = AddAdbPrefix(para->strCmd);
	if (para->nType & CMD_READ_BACK_FILE) {
		strEchoFile.Format(TEXT("%s%d"), adb_exe_path_.c_str(), GetCurrentTime());
		//CString CMD_ECHO_FILE;
		//strEchoFile.Format(TEXT("%s%d"), CMD_ECHO_FILE, 12345678);
	...
	if (para->nType & CMD_READ_BACK_FILE) 
	{
		...
		//TRACE(TEXT("\n"));
		SendDebugMessage(no, para->strReturn);
		return TRUE;
	}
...
std::wstring CAdbInterface::ToParams(int nParam)
{
	std::wstring params;
...
ANDROID_PACKAGE_T CAdbInterface::ParsePackageLine(CString line)
{
	...
	for (int i = 0; i < length; i++) {
			...
			(i == (length - 1)))
		{
			...
			if (pos == 0) {
				...
				if (endPos == -1) {
					CString temp(part.Mid(pos + 8));
					apt.package = temp.GetBuffer();
					temp.ReleaseBuffer();
				} else {
					endPos += 3;
					CString temp(part.Mid(endPos + 1));
					apt.package = temp.GetBuffer();
					temp.ReleaseBuffer();
					CString temp2(part.Mid(pos + 8, endPos - pos - 8));
					apt.path = temp2;
					temp2.ReleaseBuffer();
				}
				part.Empty();
...
void CAdbInterface::GetPackages(int param, vector<ANDROID_PACKAGE_T> &packages)
{
	packages.clear();

	PARAM_T para;
	para.strCmd.Format(TEXT("adb shell pm list packages %s"), CAdbInterface::ToParams(param).c_str());
	para.nType = CMD_INFINITE | CMD_READ_BACK_FILE;
